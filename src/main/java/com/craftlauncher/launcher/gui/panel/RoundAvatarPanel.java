...
import com.craftlauncher.launcher.gui.util.MinecraftSkin;
...
public class RoundAvatarPanel extends JPanel {
	...
	public RoundAvatarPanel() {
		...
		setUI(new ComponentUI() {
			...
			public boolean contains(JComponent c, int x, int y) {
				...
				return new Ellipse2D.Double(MaterialShadow.OFFSET_LEFT, MaterialShadow.OFFSET_TOP, width, width)
						.contains(x, y);
			}
		});
	}
	...
	protected void paintComponent(Graphics g) {
		...
		Ellipse2D circle = new Ellipse2D.Double(MaterialShadow.OFFSET_LEFT + 2, MaterialShadow.OFFSET_TOP + 2,
				width - 4, width - 4);
		...
		if(previousSkin != null) {
			...
			g2.drawImage(previousSkin, (getWidth() - skinWidth) / 2, 18 + MaterialShadow.OFFSET_TOP, skinWidth,
					skinHeight, null);
			g2.setClip(new Ellipse2D.Double(MaterialShadow.OFFSET_LEFT + (width - avatarDiameter) / 2 - 2,
					MaterialShadow.OFFSET_TOP + (width - avatarDiameter) / 2 - 2, avatarDiameter + 4,
					avatarDiameter + 4));
		}
		...
		if(skin != null) {
			...
			g2.drawImage(skin, (getWidth() - skinWidth) / 2, 18 + MaterialShadow.OFFSET_TOP, skinWidth, skinHeight,
					null);
		}
		...
		g2.draw(new Ellipse2D.Double(MaterialShadow.OFFSET_LEFT + 2, MaterialShadow.OFFSET_TOP + 2, width - 4,
				width - 4));
	}
			...
			public void run() {
				if(previousSkin != null) {
					...
					Animator skinAnimator = new Animator.Builder(timer)
							.addTarget(PropertySetter.getTarget(RoundAvatarPanel.this, "avatarDiameter", 0,
									getWidth() - MaterialShadow.OFFSET_LEFT - MaterialShadow.OFFSET_RIGHT - 10))
