			...
			switch(e.getReason()) {
				case DID_NOT_BUY_MINECRAFT:
					MainController.getInstance()
							.displayToast(new TextToast(TranslationManager.getString("loginFailedBuyMinecraft")));
				break;
				case USER_CREDENTIALS_ARE_WRONG:
					MainController.getInstance()
							.displayToast(new TextToast(TranslationManager.getString("loginFailedCredentials")));
				break;
				default:
					MainController.getInstance()
							.displayToast(new TextToast(TranslationManager.getString("loginFailed")));
				break;
			}
		...
		usernameField.addKeyListener(new KeyAdapter() {
			...
			public void keyReleased(KeyEvent e) {
				if(e.getKeyCode() == KeyEvent.VK_ENTER)
					...
					loginButton
							.setEnabled(!usernameField.getText().isEmpty() && passwordField.getPassword().length > 0);
			}
		});
