package com.craftlauncher.launcher.logic.minecraft;
...
import com.craftlauncher.launcher.logic.resources.version.VersionHelper;
import com.craftlauncher.util.JVMArgmuments;
...
public class MinecraftProcess {
	...
	private void addMinecraftPaths() {
		String natives =
				mInfo.getMinecraftPath().getMinecraftJarPath() + mInfo.getMinecraftVersion() + "-natives-131231";
		String libraries =
				VersionHelper.getLibFilessAsArgmument(mInfo.getMinecraftPath(), mVersion.getLibraries()) + "";
	...
	private void addMinecraftArguments() {
		...
		ArrayList<String> mcArgs = new ArrayList<>(Arrays.asList(mVersion.getMinecraftArguments().split(" ")));
		...
		if(mcArgs.indexOf("${game_assets}") >= 0)
			mcArgs.set(mcArgs.indexOf("${game_assets}"),
					mInfo.getMinecraftPath().getResourcePath() + "virtual" + fs + "legacy");
