...
import com.craftlauncher.launcher.logic.download.loader.JarDownloader;
import com.craftlauncher.launcher.logic.download.loader.LibraryDownloader;
import com.craftlauncher.launcher.logic.download.loader.RessourceDownloader;
import com.craftlauncher.launcher.logic.download.loader.SkinDownloader;
...
public class DownloadService implements Runnable {
	...
	private void downloadRessources() {
		...
		try {
			...
			// Sonderfall kein finRes=false da Minecraft auch mit nicht vollstaendigen Ressourcen gestartet werden kann
			// / soll.
	...
	public void setMinecraftVersion(MinecraftVersion version, String assetsVersion) throws CraftenLogicException {
		try {
			DownloadHelper.downloadFileToDiskWithoutCheck(
					DownloadUrls.URL_VERSION + assetsVersion + "/" + assetsVersion + ".json",
					mMinecraftPath.getMinecraftVersionsDir() + version.getVersion() + File.separator,
					version.getVersion() + ".json");

			Version vers = JSONReader.readJsonFileFromSelectedVersion(
					mMinecraftPath.getMinecraftJarPath() + version.getVersion() + ".json");
