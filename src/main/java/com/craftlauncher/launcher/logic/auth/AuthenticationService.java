package com.craftlauncher.launcher.logic.auth;
...
import com.craftlauncher.launcher.exception.CraftenAuthenticationException;
import com.craftlauncher.launcher.exception.CraftenException;
import com.craftlauncher.launcher.logic.json.JSONConnector;
...
public class AuthenticationService {
	...
	public String getSessionID(MinecraftUser user) throws CraftenException {
		if(user.hasAccessToken()) {
			if(refresh(user)) {
				...
				throw new CraftenAuthenticationException(
						CraftenAuthenticationException.Reason.USER_CREDENTIALS_ARE_WRONG);
		}
		else {
			...
			if(response != null && !response.equals("")) {
				...
				if(jsonObject.has("selectedProfile")) {
					this.user =
							new MinecraftUser(user.getEmail(), getProfileIDFromResponse(response), getName(response),
									getAccessTokenFromResponse(response), getClientTokenFromResponse(response));
					...
					throw new CraftenAuthenticationException(
							CraftenAuthenticationException.Reason.DID_NOT_BUY_MINECRAFT);
			}
			else
				throw new CraftenAuthenticationException(
						CraftenAuthenticationException.Reason.USER_CREDENTIALS_ARE_WRONG);
	...
	private String getSSID(String username, String password) throws CraftenAuthenticationException {
		if(username != null && !username.equals("") && password != null && !password.equals("")) {
			...
			try {
				...
				throw new CraftenAuthenticationException(
						CraftenAuthenticationException.Reason.USER_CREDENTIALS_ARE_WRONG, e);
			}
		}
		else
			return null;
	 ...
	 * Note: this method will not respond successfully to all currently-logged-in sessions, just the most
	 * recently-logged-in for each user.
	 * It is intended to be used by servers to validate that a user should be connecting
	 * (and reject users who have logged in elsewhere since starting Minecraft), NOT to auth that a particular session
	 * token is valid for authentication purposes.
	...
	public static boolean isValid(String accessToken) {
		...
		String dummy =
				JSONConnector.executePost("https://authserver.mojang.com/validate", gson.toJson(jsonAccessToken));
		return dummy != null;
	}
	...
	private boolean refresh(MinecraftUser user) {
		if(user.getAccessToken() != null && !user.getAccessToken().equals("") && user.getClientToken() != null
				&& !user.getClientToken().equals("")) {
			...
			String response =
					JSONConnector.executePost("https://authserver.mojang.com/refresh", gson.toJson(jsonPayload));
