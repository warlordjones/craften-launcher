...
public final class OSHelper {
	...
	public static String getOSArch() {
		...
		else if(wow64Arch != null && wow64Arch.endsWith("64"))
			realArch = "64";
		else
			realArch = "32";
		return realArch;
	...
	public static String getMinecraftPath() {
		...
		if(operatingSystem.equals("windows")) {
			...
			path = System.getProperty("user.home") + pS + ".minecraft" + pS;
			if(new File(path).exists())
				return path;
		}
		else if(operatingSystem.equals("mac") || operatingSystem.equals("osx")) {
			path = System.getProperty("user.home") + pS + "Library" + pS + "Application Support" + pS + "minecraft"
					+ pS;
			if(new File(path).exists())
				return path;
		}
